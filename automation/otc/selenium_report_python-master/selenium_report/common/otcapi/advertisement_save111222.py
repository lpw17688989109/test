...
class OTC(unittest.TestCase):
    ...
    def test_maidan(self, data):
        ...
        headers['token'] = config_read.configread(
            "D:\\自动化\\otc\\selenium_report_python-master\\selenium_report\\config", "new.ini", "token"+str(body['remark']),
            "token"+str(body['remark']))

        body['userId'] = config_read.configread(
        ...
        test_nub = data['id']
        #是否需要加资产
        balance = select_usercoin.select_usercoin(userid)
        if balance[0]<100 :
            update_usercoin.update_usercoin()
