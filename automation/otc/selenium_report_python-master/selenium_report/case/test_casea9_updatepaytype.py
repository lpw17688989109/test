...
testData = ExcelUtil("D:\\自动化\\otc\\canshu.xlsx", "修改收款方式").dict_data()
# print(testData)
s = requests.session()
...
class OTC(unittest.TestCase):
    ...
    def test_maidan(self, data):
        ...
        headers['token'] = config_read.configread(
            ...
            "token" + str(body['payeeName']),
            "token" + str(body['payeeName']))
        userid = config_read.configread(
            "D:\\自动化\\otc\\selenium_report_python-master\\selenium_report\\config", "new.ini",
            "uid" + str(body['payeeName']),
            "uid" + str(body['payeeName']))
        c = db.dbuser().cursor()
        c.execute('select id,user_id,payee_name from tb_user_payment_method where user_id  = %s',
                  (userid))
        strs2 = c.fetchall()
        body['id'] = strs2[int(body['payType'] - 1)][0]
        c.close()
        test_nub = data['id']
